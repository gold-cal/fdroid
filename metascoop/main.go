...
func main() {
	...
	if !*debugMode {
		...
		cmd := exec.Command("python3", "../metascoop/fdroid", "update", "--pretty", "--create-metadata", "--delete-unknown")
	...
	if !*debugMode {
		...
		cmd := exec.Command("python3", "../metascoop/fdroid", "update", "--pretty", "--delete-unknown")
		cmd.Stderr = os.Stderr
		cmd.Stdout = os.Stdout
		cmd.Stdin = os.Stdin
		cmd.Dir = filepath.Dir(*repoDir)
